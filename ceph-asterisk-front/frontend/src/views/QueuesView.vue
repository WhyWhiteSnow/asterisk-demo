...
const loadQueues = async () => {
  ...
  errorMessage.value = ''
  try {
    queues.value = await queuesApi.getQueues(selectedInstanceId.value)
  } catch (err: unknown) {
    let msg = 'Ошибка загрузки очередей'
...
const saveQueue = async () => {
  ...
  try {
    if (editingQueue.value) {
      ...
      await queuesApi.updateQueue(selectedInstanceId.value, editingQueue.value.name, updateData)
      toast.addToast({ message: 'Очередь обновлена', type: 'success' })
    } else {
      await queuesApi.createQueue(selectedInstanceId.value, form.value)
      toast.addToast({ message: 'Очередь создана', type: 'success' })
    }
    showModal.value = false
    await loadQueues()
  } catch (err: unknown) {
    let msg = editingQueue.value ? 'Ошибка обновления' : 'Ошибка создания'
...
const confirmDelete = async (queue: QueueResponse) => {
  ...
  if (!selectedInstanceId.value) return
  try {
    await queuesApi.deleteQueue(selectedInstanceId.value, queue.name)
